...
# Шаблон тега ECS7: 1-4 цифры, опционально -цифры, 2-3 буквы, 2-4 цифры,
# далее буквы/цифры/подчёркивания. Допускает "-" между группами.
RE_TAG = re.compile(r'\d{1,4}(?:[-]\d{2,4})?[-]?[A-Za-z]{2,3}[-]?\d{2,4}[-]?[A-Za-z0-9_]*')
